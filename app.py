...
for base_filename in BASE_FILENAMES:
    ...
    with open(base_filename.format("story"), 'r',encoding='utf-8',errors='replace') as file:
        story = "".join(file.readlines())
    with open(base_filename.format("summary"), "r",encoding='utf-8',errors='replace') as file:
    ...
    with open(INSTRUCTIONS_FILENAME, 'a',encoding='utf-8',errors='replace') as file:
        file.write(json.dumps({"text": INSTRUCTION}) + "\n")
    with open(STORIES_FILENAME, 'a',encoding='utf-8',errors='replace') as file:
        file.write(json.dumps({"text": story}) + "\n")
    with open(SUMMARIES_FILENAME, 'a',encoding='utf-8',errors='replace') as file:
